...
FormatoI:	addi $sp, $sp, 4			# Libero lo stack
		...
		la $a0, Resultstr			# Carico Resultstr in $a0
		li $a1, 31				# Carico dentro $a1 l'"indirizzo" da cui voglio che il controllo dei 16 bit parta
		li $a2, 16				# Carico in $a2, il numero che indica alla procedura "LetturaCampo" da quanti bit quel campo e' composto
		jal LetturaCampo			# Chiamo la procedura per leggere il campo ADDRESS e convertire i bit contenuti in decimale
		add $a0, $zero, $v0			# Metto il risultato decimale di ADDRESS dentro $a0
		jal Print16				# Chiamo la procedura per far stampare il valore decimale corrispondente al codice immesso in ADDRESS
...
##############################################################################################################
###### Procedura che controlla, prima se il primo bit del campo IMMEDIATE e' 1, e se lo e' complementa a 2 e poi stampa il campo in decimale #######
...
SedicEquals1:	addi $sp, $sp, -4			# Alloco spazio nello stack
		...
		lb $t0, 16($a0)				# Carico il primo byte del campo IMMEDIATE (bit di segno)
		bne $t0, 49, EsciSedic1		# Se il bit di segno e' 1 proseguo, se e' 0 proseguo senza complementare il numero del campo IMMEDIATE

GotoCompl: 	jal Conta1				# Chiamo la procedura per contare gli 1 presenti nel campo IMMEDIATE
		la $a0, Resultstr			# Carico in $a0 l'indirizzo della stringa binaria immessa
		add $a1, $zero, $v0			# Carico il numero degli 1 contati dentro $a1 per passarlo alla procedura Complemento2
		jal Complemento2			# Chiamo la procedura per complementare a 2 il campo IMMEDIATE
		la $a0, Resultstr			# Carico in $a0 l'indirizzo della stringa binaria immessa
		li $a1, 31				# Carico in $a1, il punto di partenza da cui iniziare la lettura dei 16 bit del campo IMMEDIATE
		li $a2, 15				# Carico in $a2, il numero che indica alla procedura "LetturaCampo" da quanti bit quel campo e' composto
		jal LetturaCampo			# Chiamo la procedura per leggere il campo IMMEDIATE e convertire i bit contenuti in decimale
		add $a0, $zero, $v0			# Copio il valore decimale restituito dalla conversione del campo IMMEDIATE in $a0
...
EsciSedic1:	la $a0, Resultstr			# Carico in $a0 l'indirizzo della stringa binaria immessa
		...
		li $a2, 15				# Carico in $a2, il numero che indica alla procedura "LetturaCampo" da quanti bit quel campo e' composto
		jal LetturaCampo			# Chiamo la procedura per leggere il campo IMMEDIATE e convertire i bit contenuti in decimale
		add $a0, $zero, $v0			# Metto il risultato decimale di IMMEDIATE dentro $a0
		jal Print16				# Chiamo la procedura per far stampare il valore decimale corrispondente al codice immesso in IMMEDIATE
		lw $ra, 0($sp)				# Ripristino $ra dallo stack
		addi $sp, $sp, 4			# Resetto lo stack
		jr $ra					# Torno alla riga successiva a "jal SedicEquals1"
...
##############################################################################################################
################################ Procedura che conta il numero degli 1 presenti negli ultimi 16 bit ##############################
...
##############################################################################################################
################################### Procedura che complementa a 2 il campo IMMEDIATE ###################################


Complemento2:	add $t1, $zero, $a1		# Copio il numero degli 1 contati nel campo IMMEDIATE in $t1
...
FormatoI3: 	la $a0, Resultstr			# Carico Resultstr in $a0
		...
		la $a0, Resultstr			# Carico Resultstr in $a0
		li $a1, 10				# Carico dentro $a1 l'"indirizzo" da cui voglio che il controllo dei 5 bit parta
		li $a2, 5				# Carico in $a2, il numero che indica alla procedura "LetturaCampo" da quanti bit quel campo è composto
		jal LetturaCampo			# Chiamo la procedura per leggere il campo RS e convertire i bit contenuti in decimale
