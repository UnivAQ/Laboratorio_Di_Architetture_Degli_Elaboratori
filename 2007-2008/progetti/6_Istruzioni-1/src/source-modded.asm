.data
	...
	messageErrorOpCode:		.asciiz "<il codice operativo immesso non è valido> (in decimale): " # HACKED
	messageErrorFuncCode:	.asciiz "<il codice funzione immesso non è valido> (in decimale): " # HACKED
...
## <Main>
	...
	main_start:
		...
		la $a3, messageErrorOpCode	# HACKED
		jal dispatchWork
		##
...
## <convertBinFromStringToDec>
	...
	_convertBinFromStringToDec_modAndSign:
		...
		# if (currentAddress != stringBaseAddress) _convertBinFromStringToDec_modAndSign
		##
		bne $t0, $t1, _convertBinFromStringToDec_modAndSign
		##
...
## <getInstructionHandler>
	...
	# @param integer $a2	Error message HACKED
	...
	_getInstructionHandler_error:
		move $s0, $a1
		#la $a0, messageErrorInstruction	# HACKED
		move $a0, $a2	# HACKED
		jal print_string
		move $a0, $s0	# HACKED
		jal print_integer
...
## <dispatchWork>
    ...
    # @param pointer $a3	Bad instruction message address HACKED
	# @return void
	##
	dispatchWork:
		addi $sp, $sp, -20	# HACKED
		sw $ra, 0($sp)
		sw $s0, 4($sp)
		sw $s1, 8($sp)
		sw $s2,12($sp)
		...
		move $s3, $a3		# bad instruction message address HACKED
		...
		move $a2, $s3		# bad instruction message address HACKED
...
## <InstructionsHandlers>
	...
	_instructionZero:
		...
		la $a3, messageErrorFuncCode	# HACKED
		jal dispatchWork
